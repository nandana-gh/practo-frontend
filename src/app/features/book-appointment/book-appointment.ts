...
export class BookAppointmentComponent implements OnInit {
  ...
  fetchDoctorProfile(): void {
    this.http.get(`http://localhost:5016/api/doctor/${this.doctorId}`).subscribe({
      next: (res: any) => {
  ...
  fetchAvailability(): void {
    ...
    this.http.get(`http://localhost:5016/api/doctor/${this.doctorId}/availability?startDate=${today}&days=7`).subscribe({
      next: (res: any) => {
  ...
  onSubmit(): void {
    ...
    this.http.post('http://localhost:5016/api/appointment/book', payload).subscribe({
      next: (res: any) => {
