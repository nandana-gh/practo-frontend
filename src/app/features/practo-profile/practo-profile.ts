...
export class PractoProfileComponent implements OnInit {
  ...
  fetchSpecialties(): void {
    this.http.get('/api/search/specialties?limit=100').subscribe({
      next: (res: any) => {
  ...
  onSubmit(): void {
    ...
    this.http.post('/api/providers/profile-lead', this.leadForm.value).subscribe({
      next: () => {
  ...
  onVerifyOtp(): void {
    ...
    this.http.post('/api/providers/register-doctor', payload).subscribe({
      next: () => {
